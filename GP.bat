...
@REM powershell -Command "$tempDir = New-Item -Type Directory -Path (Join-Path $env:temp ('TempZip_'+ (Get-Random))) -Force; try { $files = @(Get-ChildItem -Recurse -Include *.in, *.out, std.cpp, gen.cpp, validator.cpp); if ($files.Count -gt 0) { $files | Copy-Item -Destination $tempDir.FullName -Force; $stdDir = New-Item -Path ($tempDir.FullName+'\std') -Type Directory -Force; Copy-Item 'std.cpp', 'gen.cpp', 'validator.cpp' -Destination $stdDir.FullName -ErrorAction 0; Compress-Archive -Path ($tempDir.FullName+'\*') -DestinationPath ('%output_dir%\%std_zip%') -Force; echo [OK] Structured ZIP: %output_dir%\%std_zip% } else { echo [ERROR] No files found } } finally { Remove-Item $tempDir -Recurse -Force -ErrorAction 0 }"
@REM 
